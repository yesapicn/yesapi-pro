...
YesApi Pro，是一款开发和管理接口的软件产品和开发框架，企业可以基于YesApi Pro快速搭建BaaS项目、PaaS项目和SaaS项目，也可以使用YesApi Pro开发接口给内部产品或外部客户调用。  

云平台的整体架构及业务流程如下：  
![](/img/1659335435691.jpg)  
...
这三部分由YesApi Pro提供，可进行二次开发。   

开发者在接入开放平台后，根据自己的需要，开发自己的客户端应用。  
此外，开放平台可以接入或需要依赖于企业已有的系统，包括但不限于企业已有的业务系统、数据库、信息库，此部分不属于YesApi Pro提供。  
...
> API开放接口在线文档地址：http://你的域名/api/swagger-ui/index.html# 
